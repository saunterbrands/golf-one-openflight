...
if [ ! -s "$LABWC_DIR/rc.xml" ]; then
    ...
    xmlstarlet ed -P -L \
        ...
        -s '/labwc:openbox_config' -t elem -n touch -v '' \
        -i '/labwc:openbox_config/touch[last()]' -t attr \
            -n deviceName -v 'Goodix Capacitive TouchScreen' \
        -i '/labwc:openbox_config/touch[last()]' -t attr \
            -n mapToOutput -v 'DSI-2' \
        -i '/labwc:openbox_config/touch[last()]' -t attr \
            -n mouseEmulation -v 'no' \
